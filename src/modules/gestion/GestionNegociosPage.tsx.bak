...
export function GestionNegociosPage({ permissions = [] }: { permissions?: string[] }) {
  ...
  useEffect(() => {
    ...
    const unsubscribe = onSnapshot(q, (snapshot) => {
      const activeMethods = snapshot.docs
        ...
        .filter(m => m.aplicaNegocios !== false && m.estado !== 'inactivo')
        .map(m => m.nombre);
      if (activeMethods.length === 0) {
        setMetodosPago(['Transferencia', 'Transferencia-Exterior', 'Transferencia-Especial', 'Transferencia-Efectivo', 'Transferencia-Saldo']);
      } else {
  ...
  const [formData, setFormData] = useState<any>({
    ...
    direccion: '',
    urlExterno: '',
    cuentaCUP: { titular: '', cuenta: '', banco: '', sucursal: '' },
    cuentaPersonal: { titular: '', cuenta: '', banco: '', sucursal: '' },
    cuentaCheque: { titular: '', cuenta: '', banco: '' },
  ...
  // Comprehensive Filter State for all 10 fields
  ...
  // Permission checks
  const canWrite = permissions.includes('all') || permissions.includes('gestion-negocios:write');
  ...
  const filteredData = useMemo(() => {
    ...
    return data.filter(item => {
      ...
      if (filters.nombreContrato && !(item.nombreContrato || item.noContrato || '').toLowerCase().includes(filters.nombreContrato.toLowerCase())) return false;
  ...
  const handleExportExcel = () => {
    ...
    const rows = filteredData.map(item => [
      ...
      `"${(item.nombreContrato || item.noContrato || '').replace(/"/g, '""')}"`,
      `"${(item.nit || '').replace(/"/g, '""')}"`,
            ...
            <tbody>
              ${filteredData.map(item => `
                <tr>
                  ...
                  <td>${item.telefono || '-'}</td>
                  <td>${item.correo || '-'}</td>
                  <td>${item.nombreContrato || item.noContrato || '-'}</td>
                  <td>${item.nit || '-'}</td>
  ...
  const handleEdit = (business: BusinessRecord) => {
    ...
    setFormData({
      ...
      area: business.area || 'La Habana',
      contacto: business.contacto || '',
      telefono: business.telefono || '',
      correo: business.correo || '',
      noContrato: business.noContrato || '',
      nombreContrato: business.nombreContrato || business.noContrato || '',
      nit: business.nit || '',
      direccion: business.direccion || '',
      urlExterno: business.urlExterno || '',
      cuentaCUP: business.cuentaCUP || { titular: '', cuenta: '', banco: '', sucursal: '' },
      cuentaPersonal: (business as any).cuentaPersonal || { titular: '', cuenta: '', banco: '', sucursal: '' },
      cuentaCheque: (business as any).cuentaCheque || { titular: '', cuenta: '', banco: '' },
  ...
  const handleSave = async () => {
    try {
      const businessData = {
        ...formData,
        noContrato: formData.noContrato || formData.nombreContrato || '',
        updatedAt: serverTimestamp(),
      };

      if (selectedBusiness) {
        await updateDoc(doc(db, 'businesses', selectedBusiness.id), businessData);
  ...
  const handleConfirmDelete = async () => {
    ...
    try {
      ...
      await deleteDoc(doc(db, 'businesses', businessIdToDelete));
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="space-y-4">
        ...
        {/* Expandable Comprehensive Filters Panel */}
        <AnimatePresence>
          {showFilters && (
            <motion.div
          ...
          },
          {
            ...
            accessor: (item: BusinessRecord) => (
              <div className="flex flex-col text-xs font-mono">
                <span className="text-[var(--color-text)]">{item.nombreContrato || item.noContrato || 'N/A'}</span>
      ...
      >
        <div className="space-y-8">
           ...
           <FormSection title="Información Operativa">
              <div className="space-y-4">
                 <DetailItem icon={MapPin} label="Provincia/Área" value={selectedBusiness?.area} />
                 <DetailItem icon={Banknote} label="Método de Pago" value={selectedBusiness?.metodoPago} />
                 <DetailItem icon={Hash} label="Nombre Contrato / No. Contrato" value={selectedBusiness?.nombreContrato || selectedBusiness?.noContrato} />
      ...
      <SlideOver
        ...
        footer={
          <div className="flex gap-3">
             ...
             <Button variant="brand" className="flex-1" onClick={handleSave}>Guardar Negocio</Button>
          </div>
        }
      >
             ...
             <div className="grid grid-cols-1 gap-4">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                   <TextField 
                     ...
                     value={formData.nombreContrato || formData.noContrato} 
                     onChange={(e) => setFormData(prev => ({ ...prev, nombreContrato: e.target.value, noContrato: e.target.value }))}
          ...
          {showExteriorSection && (
            ...
            >
               <div className="space-y-6">
                  ...
                  {/* Transferencia Internacional Option */}
                  <div className="p-4 bg-white border border-[var(--color-border)] rounded-xl relative overflow-hidden shadow-sm space-y-4">
                     <label className="flex items-center gap-2 cursor-pointer">
                        <input 
                           type="checkbox" 
