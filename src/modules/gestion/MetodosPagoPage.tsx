...
import { CreditCard, Plus, Edit2, Trash2, Loader2, Save, Search, CheckSquare, Square } from 'lucide-react';
...
export function MetodosPagoPage({ permissions = [], type }: { permissions?: string[]; type?: 'negocios' | 'mensajeros' | 'ordenes' }) {
  ...
  const canWrite = permissions.includes('all') || 
                   permissions.includes('metodos-pago:write') || 
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    try {
      if (selectedMethod) {
        await updateDoc(doc(db, 'MetodosPago', selectedMethod.id), {
          ...formData,
  ...
  const handleConfirmDelete = async () => {
    ...
    try {
      await deleteDoc(doc(db, 'MetodosPago', methodIdToDelete));
    } catch (err) {
  ...
  // Only show the "Uso en Sub-sistemas" column if there's no specific type active
  if (!type) {
    tableColumns.push({
      ...
      <SlideOver
        ...
        footer={
          <div className="flex gap-3">
            ...
            <Button variant="brand" className="flex-1" onClick={handleSave} disabled={!formData.nombre}>
