...
export function LogsAuditoriaPage({ permissions = [] }: { permissions?: string[] }) {
  ...
  useEffect(() => {
    // Escuchar cambios en la base de datos para la colección AuditLogs
    const q = query(collection(db, 'AuditLogs'));
    
    const unsubscribe = onSnapshot(q, (snapshot) => {
      const records = snapshot.docs.map(docSub => {
        const item = docSub.data();
        return {
          id: docSub.id,
          module: item.module || 'Otros',
          action: item.action || 'Acción',
          user: item.user || 'Sistemas',
  ...
  // Filter and process the records local or cloud
    ...
    return data.filter(item => {
      ...
      if (item.timestamp) {
        if (item.timestamp instanceof Timestamp) {
          logTime = item.timestamp.toDate();
        } else if (item.timestamp.seconds) {
          logTime = new Timestamp(item.timestamp.seconds, item.timestamp.nanoseconds).toDate();
        } else {
          logTime = new Date(item.timestamp);
        }
      }
      ...
      if (logTime) {
        ...
        // If dates are filtered but log lacks a timestamp, skip
        return false;
      }
  ...
  const sortedData = useMemo(() => {
    ...
    sorted.sort((a, b) => {
      ...
      if (sortField === 'timestamp') {
        const aTime = aVal ? (aVal.seconds ? aVal.seconds * 1000 + (aVal.nanoseconds / 1000000) : new Date(aVal).getTime()) : 0;
        const bTime = bVal ? (bVal.seconds ? bVal.seconds * 1000 + (bVal.nanoseconds / 1000000) : new Date(bVal).getTime()) : 0;
  ...
  const formatLogTimestamp = (ts: any): string => {
    ...
    try {
      ...
      if (ts.seconds) {
        return formatDatetime(new Timestamp(ts.seconds, ts.nanoseconds).toDate());
      }
  ...
  const getModuleBadgeColor = (module: string) => {
    const m = (module || '').toLowerCase().normalize("NFD").replace(/[\u0300-\u036f]/g, "");
    ...
    <div className="space-y-6">
      ...
      {loading ? (
        <div className="bg-[var(--color-surface)] border border-[var(--color-border)] rounded-[var(--radius-md)] p-12 text-center flex flex-col items-center justify-center">
          ...
          <p className="text-sm text-[var(--color-text-muted)]">Cargando bitácora de auditoría descentralizada...</p>
        </div>
        ...
        <div className="bg-[var(--color-surface)] border border-[var(--color-border)] rounded-[var(--radius-md)] overflow-hidden shadow-sm flex flex-col">
          {/* Table Header Wrapper to ensure standard responsive grid overflow */}
          ...
          {/* Standard neat minimal pagination block exactly matching Verification flow */}
          {totalPages > 1 && (
