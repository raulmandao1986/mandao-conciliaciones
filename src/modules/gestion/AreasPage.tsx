...
import { MapPin, Plus, Edit2, Trash2, Loader2, Save, X, Search, Map } from 'lucide-react';
...
export function AreasPage({ permissions = [] }: { permissions?: string[] }) {
  ...
  const canWrite = permissions.includes('all') || 
                   permissions.includes('areas:write') || 
  ...
  const handleSave = async (e?: React.FormEvent | React.MouseEvent) => {
    ...
    try {
      if (selectedArea) {
        await updateDoc(doc(db, 'Areas', selectedArea.id), {
          ...formData,
  ...
  const handleConfirmDelete = async () => {
    ...
    try {
      await deleteDoc(doc(db, 'Areas', areaIdToDelete));
    } catch (err) {
          ...
          <div className="space-y-4">
            ...
            <label className="flex flex-col gap-1.5">
              ...
              <span className="text-[10px] text-[var(--color-text-muted)] block leading-normal">
                Indique el ID único del documento de Google Sheets para este Dispatcher. (Se obtiene de la URL de tu hoja de cálculo).
              </span>
            </label>
